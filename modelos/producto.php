...
class Producto{
    ...
    public function Listar(){
        try{
            $consulta=$this->pdo->prepare("SELECT * FROM productos JOIN marcas on productos.pro_mar = marcas.mar_id;");
            $consulta->execute();
    ...
    public function Insertar(Producto $p){
        try{
            $consulta="INSERT INTO productos(pro_nom,pro_mar,pro_cos,pro_pre,pro_can,pro_img)
             VALUES (?,?,?,?,?,?);";
    ...
    public function ListarMarcas(){
        try{
            $consulta = $this->pdo->prepare("SELECT * FROM marcas;");
            $consulta->execute();
            return $consulta->fetchAll(PDO::FETCH_OBJ);
        }   catch(Exception $e){
            die($e->getMessage());
